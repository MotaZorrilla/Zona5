...
class SeguimientoProgresoReporte {
    ...
    async obtenerEstadoTarea() {
        try {
            ...
            if (!contentType || !contentType.includes('application/json')) {
                throw new Error('La respuesta no es JSON - probablemente un error del servidor');
            }
            ...
            // Verificar si el error es específicamente de parsing JSON
            if (error instanceof SyntaxError || error.message.includes('JSON')) {
                console.error(`%c❌ La respuesta del servidor no es un JSON válido. Puede haber un error en el servidor.`, 'color: #dc3545;');
            }
    ...
    actualizarProgreso(datosTarea) {
        ...
        // Verificar si la tarea está estancada en 10% (Iniciando procesamiento)
        // y si está en estado de procesamiento, iniciar el proceso real
        if (progreso === 10 && estado === 'processing' && message.includes('Iniciando')) {
            this.iniciarProcesamientoReal();
        }
    ...
    /**
     * Iniciar el procesamiento real de la tarea
     */
    async iniciarProcesamientoReal() {
        try {
            const response = await fetch('/admin/reports/start-processing', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content')
            ...
            if (data.success) {
                console.log(`%c✅ Procesamiento real iniciado para la tarea ${this.taskId}`, 'color: #28a745; font-weight: bold;');
            } else {
                console.error(`%c❌ Error al iniciar procesamiento real:`, 'color: #dc3545;', data.message);
            }
        } catch (error) {
            console.error(`%c❌ Error al llamar a iniciar procesamiento real:`, 'color: #dc3545;', error);
        }
    }
