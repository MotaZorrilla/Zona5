...
<head>
    ...
    <style>
        :root {
            ...
            --light-bg: #F3F4F6; /* Gray 10 original */
        ...
        .sidebar-link.active {
            ...
            color: white !important;
            font-weight: 600;
            box-shadow: 0 4px 6px -1px rgba(9, 102, 241, 0.4), 0 2px 4px -1px rgba(99, 102, 241, 0.2);
        }
        ...
        <div class="p-6">
            ...
            <div class="py-2 px-6 bg-gradient-to-r from-indigo-700 to-purple-800 flex items-center shadow-sm rounded-lg sticky top-6 z-30">
                ...
                @if(auth()->user() && auth()->user()->isPending())
                    <!-- Barra superior minimal para usuarios pendientes -->
                    <div class="flex items-center">
                        <a href="{{ route('welcome') }}" target="_blank" rel="noopener" class="flex items-center">
                            <x-application-logo class="w-8 h-8 rounded-full bg-white flex items-center justify-center text-primary-500 text-lg font-bold" />
                            <span class="text-lg font-bold text-white ml-2">Gran Zona 5</span>
                        </a>
                    </div>
                    
                    <div class="ml-auto flex items-center" x-data="{ profileOpen: false }">
                        <!-- Notifications -->
                        <livewire:notifications />
                        <div class="relative" @click.away="profileOpen = false">
                            <button @click="profileOpen = !profileOpen" type="button" class="flex items-center">
                                <img src="{{ auth()->user()->profile_photo_path ?? 'https://ui-avatars.com/api/?name='.urlencode(auth()->user()->name).'&color=1e293b&background=f1f5f9' }}" alt="{{ auth()->user()->name }}" class="w-8 h-8 rounded-full object-cover">
                            </button>
                            <div x-show="profileOpen" class="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-xl py-1 z-20">
                                <div class="px-4 py-2 border-b bg-gray-50">
                                    <p class="text-sm font-semibold text-gray-80">{{ auth()->user()->name }}</p>
                                    <p class="text-xs text-gray-50">Usuario Pendiente</p>
                                </div>
                                <a href="{{ route('profile') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-indigo-50">Mi Perfil</a>
                                <a href="{{ route('profile') }}" class="block px-4 py-2 text-sm text-gray-70 hover:bg-indigo-50">Configurar Perfil</a>
                                <hr class="my-1">
                ...
                @else
                    <!-- Barra superior completa para usuarios aprobados -->
            ...
            <div class="mt-6">
                @if (session('success'))
                    <div class="mb-6 bg-green-10 border-l-4 border-green-500 text-green-700 p-4 rounded-lg shadow-md" role="alert">
                ...
                @if (session('error'))
                    <div class="mb-6 bg-red-100 border-l-4 border-red-50 text-red-700 p-4 rounded-lg shadow-md" role="alert">
