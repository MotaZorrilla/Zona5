...
@section('title', 'Crear Dignatario de Zona')

@section('content')
    <div class="flex justify-between items-center mb-8">
        <div>
            <h1 class="text-3xl font-bold text-gray-800">Crear Dignatario de Zona</h1>
            <p class="text-sm text-gray-500 mt-1">Completa los detalles para añadir un nuevo dignatario de la Zona 5.</p>
        </div>
    ...
    <div class="bg-white p-8 rounded-xl shadow-lg">
        <form action="{{ route('admin.zone-dignitaries.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="role" id="role" class="w-full bg-white border-2 border-gray-200 rounded-lg py-2 px-4 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-colors" required>
                    </div>
                    <div>
                        <label for="image_url" class="block text-sm font-medium text-gray-700 mb-1">URL de la Imagen (Opcional)</label>
                        <input type="url" name="image_url" id="image_url" class="w-full bg-white border-2 border-gray-200 rounded-lg py-2 px-4 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-colors">
                    </div>
                    <div>
                        <label for="bio" class="block text-sm font-medium text-gray-700 mb-1">Biografía (Opcional)</label>
                        <textarea name="bio" id="bio" rows="4" class="w-full bg-white border-2 border-gray-200 rounded-lg py-2 px-4 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-colors"></textarea>
                    </div>
                </div>
            </div>
