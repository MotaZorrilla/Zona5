...
<div class="bg-white p-8 rounded-xl shadow-lg">
    ...
    <div class="flex justify-between items-center mb-8">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Completa la información para programar un nuevo evento.</p>
        </div>
    ...
    <form action="{{ route('admin.events.store') }}" method="POST" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
                ...
                <select name="type" id="type" 
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                        required>
                    <option value="event" {{ old('type') == 'event' ? 'selected' : '' }}>Evento General</option>
                    ...
                    <option value="tenida" {{ old('type') == 'tenida' ? 'selected' : '' }}>Tenida</option>
                </select>
                @error('type')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
        ...
        <div class="flex justify-end pt-4">
            <button type="submit" 
                    class="flex items-center bg-primary-500 hover:bg-primary-600 text-white font-bold py-2 px-6 rounded-lg shadow-md transition-transform transform hover:scale-105">
