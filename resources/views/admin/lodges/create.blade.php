...
@section('title', 'Crear Logia')

@section('content')
    <h1 class="text-3xl font-bold text-gray-800 mb-6">Crear Nueva Logia</h1>

    <div class="bg-white p-6 rounded-lg shadow-sm">
        <div class="bg-primary-600 text-white p-4 rounded-t-lg mb-6">
            <h2 class="text-xl font-bold">Información de la Logia</h2>
        </div>
        <form action="{{ route('admin.lodges.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <input type="number" name="number" id="number" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm" required>
            </div>
            <div class="mb-4">
                <label for="orient" class="block text-sm font-medium text-gray-700">Orient</label>
                <input type="text" name="orient" id="orient" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm" required>
            </div>
            <div class="mb-4">
                <label for="address" class="block text-sm font-medium text-gray-700">Dirección Completa</label>
                <textarea name="address" id="address" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm"></textarea>
            </div>
            <div class="mb-4">
                <label for="history" class="block text-sm font-medium text-gray-700">Historia (para la página pública)</label>
                <textarea name="history" id="history" rows="5" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm"></textarea>
            </div>
                        <div class="mb-4">
                <label for="image_url" class="block text-sm font-medium text-gray-700">Imagen (para la página pública)</label>
                <div x-data="{ imagePreview: null, handleDrop(e) { this.imagePreview = URL.createObjectURL(e.dataTransfer.files[0]); this.$refs.fileInput.files = e.dataTransfer.files; } }" 
