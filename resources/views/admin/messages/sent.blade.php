...
<div class="flex h-[calc(100vh-150px)] bg-gradient-to-br from-indigo-50 to-purple-50 rounded-2xl shadow-xl overflow-hidden border-gray-10">
    ...
    <aside class="w-72 bg-gradient-to-b from-indigo-700 to-purple-800 text-white p-5 flex-shrink-0 shadow-lg">
        ...
        <nav class="space-y-1 mt-6">
            ...
            <a href="{{ route('admin.messages.starred') }}" class="flex items-center px-4 py-3 text-indigo-100 hover:bg-indigo-600 rounded-xl transition-all duration-300 hover:text-white group">
                <i class="ri-star-line mr-3 text-xl group-hover:text-yellow-300 transition-colors"></i>
                <span>Destacados</span>
            </a>
            <a href="{{ route('admin.messages.sent') }}" class="flex items-center px-4 py-3 text-white font-semibold bg-indigo-600 rounded-xl mb-1 transition-all duration-300 hover:bg-indigo-500 shadow-md">
    ...
    <div class="w-2/5 border-r border-gray-200 flex flex-col bg-white shadow-inner">
        <div class="p-5 border-b border-gray-100 bg-gradient-to-r from-white to-indigo-50 sticky top-0 bg-white z-10 shadow-sm">
            ...
            <div class="relative mt-4">
                ...
                <input type="search" name="search" placeholder="Buscar en enviados..." class="w-full bg-gray-50 border-2 border-gray-200 rounded-xl py-3 pl-10 pr-4 focus:outline-none focus:ring-2 focus:ring-indigo-300 focus:border-indigo-400 transition-all shadow-sm focus:shadow-md" />
            </div>
        </div>
            ...
            @forelse ($messages as $message)
                <a href="{{ route('admin.messages.show', $message) }}" class="block p-4 border-b border-gray-100 hover:bg-gradient-to-r hover:from-indigo-50 hover:to-purple-50 transition-all duration-300 @if($message->isUnread()) bg-gradient-to-r from-indigo-50 to-blue-50 border-l-4 border-indigo-500 @else border-l-4 border-transparent @endif group relative overflow-hidden">
                    <div class="flex justify-between items-start mb-2">
                        <p class="text-base font-bold text-gray-800 truncate">{{ $message->recipient_name ?? 'Destinatario desconocido' }}</p>
                        @if($message->isUnread())
                    ...
                    <div class="flex justify-between items-center mt-3 text-xs text-gray-500">
                        ...
                        <div class="flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity absolute right-4 top-1/2 -translate-y-1/2 bg-white rounded-full p-1 shadow-md">
                            ...
                            <button title="Marcar como no leído" class="p-2 rounded-full hover:bg-blue-10 text-gray-500 hover:text-blue-600"><i class="ri-mail-unread-line"></i></button>
                        </div>
                    </div>
                </a>
            @empty
                <div class="p-10 text-center h-full flex flex-col justify-center items-center">
                    <div class="w-24 h-24 bg-indigo-100 rounded-full flex items-center justify-center mb-4">
                        <i class="ri-send-plane-line text-4xl text-indigo-400"></i>
                    </div>
                    <h3 class="mt-2 text-xl font-bold text-gray-70">No hay mensajes enviados</h3>
                    <p class="mt-1 text-gray-500">Envía un mensaje para verlo aquí.</p>
                </div>
            @endforelse
        </div>
                ...
                <div class="flex justify-between items-start mb-4">
                    <div>
                        ...
                        <div class="flex items-center mt-3">
                            ...
                            <div>
                                <div class="text-base font-semibold text-gray-800">{{ $selectedMessage->recipient_name ?? 'Destinatario desconocido' }}</div>
                                <div class="text-sm text-gray-500">{{ $selectedMessage->recipient_email ?? '' }}</div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="flex items-center justify-center h-full bg-gradient-to-br from-indigo-50 to-purple-50 p-8">
                <div class="text-center max-w-md">
                    <div class="w-32 h-32 bg-gradient-to-r from-indigo-100 to-purple-100 rounded-full flex items-center justify-center mx-auto mb-6 shadow-lg">
                        <i class="ri-send-plane-line text-5xl text-indigo-400"></i>
                    </div>
